...
-- Manual-aligned pipeline for inbound leads, quotes, won/lost outcomes, potential revenue, and realized revenue.
...
values
  ...
  ('install_closeout', 'Install & Close-Out', 'Close-Out', 50, 'won', false, true, false, 'Material, install, completion approval, final payment, and closeout are being handled.'),
...
create table if not exists public.crm_leads (
  ...
  constraint lost_reason_required check (
    stage_id <> 'lost_cancelled'
    or lost_reason is not null
  )
);

comment on column public.crm_leads.external_lead_id is 'User-facing Measure Work Order Number.';
...
create index if not exists crm_leads_stage_entered_idx on public.crm_leads (stage_entered_at);
create index if not exists crm_leads_date_received_idx on public.crm_leads (date_received);
create index if not exists crm_leads_next_action_due_idx on public.crm_leads (next_action_due);
create index if not exists crm_leads_last_activity_idx on public.crm_leads (last_activity_at);
...
create index if not exists crm_leads_archived_at_idx on public.crm_leads (archived_at);
...
create index if not exists crm_lead_activities_lead_idx on public.crm_lead_activities (lead_id, occurred_at desc);
create index if not exists crm_lead_activities_user_idx on public.crm_lead_activities (user_id, occurred_at desc);
...
create trigger crm_leads_stage_history
after insert or update of stage_id on public.crm_leads
for each row execute function public.crm_record_stage_history();
...
as $$
  ...
  rolled as (
    ...
    where exists (
      ...
      where ur.user_id = p.user_id and ur.role = 'rep'
    )
...
drop policy if exists "stages read authenticated" on public.crm_pipeline_stages;
create policy "stages read authenticated" on public.crm_pipeline_stages
for select to authenticated using (true);

drop policy if exists "stages admin manage" on public.crm_pipeline_stages;
create policy "stages admin manage" on public.crm_pipeline_stages
to authenticated using (public.has_role(auth.uid(), 'admin')) with check (public.has_role(auth.uid(), 'admin'));
...
create policy "leads insert assigned or admin" on public.crm_leads
...
create policy "leads update assigned or admin" on public.crm_leads
